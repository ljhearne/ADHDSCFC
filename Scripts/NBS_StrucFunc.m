function [MAT,max_sz,max_szNull,nbsP] = NBS_StrucFunc(ADHDSC,CTRLSC,AllFC_AC,thresh,SCthresh,perms,Path,LOAD)
%explanation goes here



%how many SC data points are needed to consider a SC-FC correlation? 62 = 80% of smallest group
% previously this parameter was set to 62 (80% of smallest group), but to
% be congruent with other analyses I think we should have every participant
% have SC values. Will test how this affects results.
rng('shuffle')
...
[Fmatrix,~] = GLM_SCFC(ADHDSC,CTRLSC,AllFC_AC,SCthresh,0);
...
if LOAD == 0
    ...
    parfor p = 1:perms
        tic
        [FmatrixNull(:,:,p),~] = GLM_SCFC(ADHDSC,CTRLSC,AllFC_AC,SCthresh,1);
        toc
    end
    
    %save the data
    out = [Path,'Results/NBSSCFC_SCthresh',num2str(SCthresh),'_Perms',num2str(perms),',.mat'];
    save(out,'FmatrixNull');
    
elseif LOAD == 1
    try
        load([Path,'Results/NBSSCFC_SCthresh',num2str(SCthresh),'_Perms',num2str(perms),',.mat']);
    catch
