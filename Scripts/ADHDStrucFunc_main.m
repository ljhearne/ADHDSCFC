...
if FIGS ==1
    ...
    %% functional network affiliations
    figure('Color','w','Position',[850 450 400 200]); hold on
    labels = {'Visual';'Sensory-motor';'Dorsal attention';'Salience';'Limbic';'Fronto-parietal';'Default-mode';'Sub-cortical'};
    for i = 1:max(Yeo8Index)
        CI = ConfInt(hublist.CTRLcount(:,i));
        h = line([CI(1),CI(2)],[i+.15,i+.15]);
        set(h,'Color',cl(1,:));
        scatter(mean(hublist.CTRLcount(:,i)),i+.15,...
            'MarkerEdgeColor',cl(1,:),...
            'MarkerFaceColor',cl(1,:),...
            'LineWidth',1); hold on;
        
        CI = ConfInt(hublist.ADHDcount(:,i));
        h = line([CI(1),CI(2)],[i-.15,i-.15]);
        set(h,'Color',cl(2,:));
        scatter(median(hublist.ADHDcount(:,i)),i-.15,...
            'MarkerEdgeColor',cl(2,:),...
            'MarkerFaceColor',cl(2,:),...
            'LineWidth',1); hold on;
    end
    
    xlabel('Mean number of SC hubs');
    %ylabel('Connectome');
    set(gca,'FontName', 'Helvetica','FontSize', 12,'box','off','Ytick',[]);
    set(gca,'YTick',1:9,'YTickLabel', labels);
    set(gca,'Xtick',0:1:9);
    saveas(gcf,[resultsdir,'Figure2_HubAffilations.jpeg']);
